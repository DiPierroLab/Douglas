...
class MiChroM:
    ...
    def setup(self, platform="CUDA", PBC=False, PBCbox=None, GPU="default",
        ...
        #header_types = range(diff_types_size)# I repurposed the types such that each locus has its own type (like the Douglas personality theory)
        Args:
...
#-------------------------------------------------------------------------------------------------------------------
    def addLambdas(self, name="Lambdas", mu=3.22, rc = 1.78, LambdasArray=None):#This method was adapted from addCustomTypes. It treats each locus as its own type like in the Douglas personality theory. 
        
        self.metadata["CrossLink"] = repr({"mu": mu})
        energyLambdas = ("lambdasFunction(bead_index1,bead_index2)*0.5*(1. + tanh(mu*(rc - r)))")

        crossLP_lambdas = self.mm.CustomNonbondedForce(energyLambdas)

        crossLP_lambdas.addGlobalParameter('mu', mu)
        crossLP_lambdas.addGlobalParameter('rc', rc)
        crossLP_lambdas.addGlobalParameter('lim', 1.0)
        crossLP_lambdas.setCutoffDistance(3.0)

        lambdas_table = pd.read_csv(LambdasArray, sep=None, engine='python', header=None)#tab in original
        if lambdas_table.shape[0] != lambdas_table.shape[1]:
            print("You didn't input a square matrix you dodo!")
            exit()
        elif lambdas_table.shape[0] != self.N:
            print("You didn't input a matrix of the right size you dodo!")
            exit()
        #np.savetxt("areLambdasSameInRAM.txt",tab)
        if not(np.allclose(lambdas_table,np.transpose(lambdas_table),1e-16,1e-16)):
            print("asymmetric input LambdasArray")
            exit()
        else:
            print("symmetric input LambdasArray")
        
        print("lambdas_table shape = ",lambdas_table.shape)
        
        header_indices = list(lambdas_table.columns.values)#range(lambdas_table.shape[0])
        diff_indices_size = len(header_indices)
        lambdas = np.triu(lambdas_table.values) + np.triu(lambdas_table.values, k=1).T
        lambdas = list(np.ravel(lambdas))
        fTypes = self.mm.Discrete2DFunction(diff_indices_size,diff_indices_size,lambdas)
        crossLP_lambdas.addTabulatedFunction('lambdasFunction', fTypes)
        #self._createTypeList(header_indices)
        crossLP_lambdas.addPerParticleParameter("bead_index")#this "bead_index" corresponds to the bead_index1 and bead_index2 in the energyLambdas thing.

        for i in range(self.N):
            value = [float(header_indices[i])]
